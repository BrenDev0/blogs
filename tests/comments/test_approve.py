...
from src.features.comments.application.use_cases.approve_one import ApproveComment
...
def test_success_approve(
    mock_comment_repository,
    mock_post_repository,
    use_case: ApproveComment
):
    user_id = uuid4()
    comment_id = uuid4()
    post_id = uuid4()
    ...
    mock_comment_repository.update.return_value = Comment(
        **fake_comment.model_dump(exclude="approved"), approved=True
    )
    ...
    mock_comment_repository.update.assert_called_once_with(
        key="comment_id",
        value=comment_id,
...
def test_success_already_approved(
    mock_comment_repository,
    mock_post_repository,
    use_case: ApproveComment
):
    ...
    mock_comment_repository.update.assert_not_called()
    assert result.comment_id == comment_id
    assert result.approved is True
